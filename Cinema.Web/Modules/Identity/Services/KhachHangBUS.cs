...
namespace Cinema.Web.Modules.Identity.Services
{
    public class KhachHangBUS : IKhachHangBUS
    {
        ...
        public KhachHangBUS(IdentityDbContext db, BookingDbContext bookingDb)
        {
            _db = db;
        ...
        public KhachHangDTO LayThongTinProfile(string taiKhoan)
        {
            ...
            var hoaDons = _bookingDb.HoaDons.Where(h => h.MaKh == kh.MaKh).ToList();
            ...
            return new KhachHangDTO
            {
                ...
                DiemTichLuy = kh.DiemTichLuy ?? 0,
                LichSuHoaDon = hoaDons.Select(h => new HoaDonDTO
                {
                    MaHD = h.MaHd,
                    NgayDat = h.NgayDat,
                    TongTien = h.TongTien,
