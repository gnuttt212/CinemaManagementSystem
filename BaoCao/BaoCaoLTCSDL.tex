...
Người dùng cuối có thể duyệt danh sách phim, chọn suất chiếu trực quan qua bộ lọc ngày tháng, chọn ghế ngồi, thêm dịch vụ bắp nước/combo, xác nhận thanh toán và nhận vé điện tử (E-Ticket) kèm mã QR. Phía quản trị viên có thể quản lý toàn bộ danh mục phim, suất chiếu, dịch vụ và xem báo cáo doanh thu theo phim thông qua biểu đồ Chart.js tích hợp. Mật khẩu người dùng được mã hóa bảo mật toàn diện bằng thuật toán an toàn tuyệt đối BCrypt.
...
\begin{table}[H]
    \begin{center}
        \begin{tabular}{|l|l|}
            ...
            \hline
            \texttt{Admin/DoAnController}      & Quản trị dịch vụ: CRUD (thêm mới trong phiên này) \\
            \hline
...
\begin{itemize}
    ...
    \item \textbf{Entity Framework:} Cấu hình song song Database First (scaffold 10 entity) và Code First (model \texttt{NhatKyHeThong} với Data Annotation). Biên dịch tự động toàn bộ Model sang mã T-SQL qua lệnh \texttt{dotnet-ef}.
    \item \textbf{Web API RESTful:} 3 Controller API (\texttt{PhimApiController}, \texttt{DoAnApiController}, \texttt{LichChieuApiController}) với các endpoint trả JSON chuẩn REST (GET/POST/PUT/DELETE).
    \item \textbf{Tính năng người dùng:} Đặt vé, chọn ghế, dịch vụ combo, thanh toán, E-Ticket QR, hồ sơ cá nhân và \textbf{đổi mật khẩu}. Lọc suất chiếu theo ngày hiện tại và tương lai trực quan.
    \item \textbf{Bảo mật hệ thống:} Mã hóa mật khẩu người dùng bằng thư viện \textbf{BCrypt} ngăn chặn băm ngược, tránh rò rỉ dữ liệu khi cơ sở dữ liệu bị lộ.
