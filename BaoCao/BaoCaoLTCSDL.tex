...
\begin{center}
\textbf{Sinh vi\^en th\d{u}c hi\d{e}n}
\end{center}
...
\begin{table}[h]
    \begin{tabular}{ll}
        ...
        Sinh viên       & Trần Thanh Tung (MSSV: 2351010232) \\
...
Người dùng cuối có thể duyệt danh sách phim, chọn suất chiếu trực quan qua bộ lọc ngày tháng, chọn ghế ngồi với cơ chế \textbf{sinh ghế động} và \textbf{khóa ghế theo thời gian thực bằng SignalR}, thêm dịch vụ bắp nước/combo, xác nhận \textbf{thanh toán trực tuyến qua cổng VNPay} và nhận vé điện tử (E-Ticket) kèm mã QR. Để tăng trải nghiệm, hệ thống hỗ trợ \textbf{đăng nhập một chạm qua Google OAuth 2.0}, giao diện hiện đại với \textbf{GSAP Animation}, điều hướng hồ sơ người dùng trực quan, và tối ưu hóa hiệu năng bằng \textbf{IMemoryCache}. Phía quản trị viên có thể quản lý toàn bộ danh mục phim, phòng chiếu, suất chiếu, dịch vụ, xuất \textbf{báo cáo doanh thu ra file Excel (ClosedXML)} và xem biểu đồ Chart.js trực quan. Hệ thống cũng được đảm bảo chất lượng thông qua việc \textbf{viết Unit Test bằng xUnit và Moq}.

Ngoài ra, hệ thống phát triển \textbf{RESTful Web API} với đầy đủ các endpoint CRUD, hỗ trợ export/import dữ liệu qua định dạng XML sử dụng \textbf{LINQ to XML}. Mật khẩu người dùng được mã hóa bảo mật toàn diện bằng thuật toán an toàn tuyệt đối BCrypt.
...
\noindent{\textbf{Từ khóa:}} ASP.NET Core MVC, Entity Framework Core, SQL Server T-SQL, ADO.NET, LINQ, LINQ to XML, RESTful Web API, Kiến trúc 3 lớp, Quản lý rạp phim, xUnit, BCrypt, VNPay, Google OAuth 2.0, SignalR, GSAP.
...
\begin{table}[H]
    \begin{center}
        \begin{tabular}{|c|l|l|}
            \hline
            \textbf{MSSV} & \textbf{Họ tên} & \textbf{Nhiệm vụ} \\
            \hline
            2351010232 & Trần Thanh Tung & Toàn bộ hệ thống (Full-stack) \\
            \hline
        \end{tabular}
    \end{center}
...
\begin{itemize}
    \item \textbf{Khách hàng:} Đăng ký, \textbf{đăng nhập bằng Google}, duyệt danh sách phim (tốc độ cao nhờ Caching), chọn suất chiếu, chọn ghế (tích hợp \textbf{SignalR} khóa ghế realtime), thêm dịch vụ, \textbf{thanh toán qua ví điện tử VNPay} và nhận E-Ticket có mã QR. Sau khi đăng nhập, người dùng có thể cập nhật Profile qua menu điều hướng trực quan và đổi mật khẩu. Giao diện được tạo hiệu ứng mượt mà với \textbf{GSAP}.
    \item \textbf{Quản trị viên:} Quản lý phim, quản lý \textbf{phòng chiếu (với cơ chế sinh ghế động theo sức chứa)}, quản lý suất chiếu (tự động tính giờ kết thúc), quản lý dịch vụ bắp nước, xem báo cáo doanh thu và \textbf{xuất báo cáo ra file Excel}.
\end{itemize}
...
\begin{table}[H]
    \begin{center}
        \begin{tabular}{|l|l|}
            ...
            \hline
            Frontend           & Razor View, Bootstrap 5.3.2, jQuery 3.7.1, GSAP \\
            \hline
            Real-time          & SignalR \\
            \hline
...
\begin{de}\label{def1}
    \textbf{Các thực thể chính} trong hệ thống bao gồm: \texttt{NhanVien}, \texttt{KhachHang}, \texttt{Phim}, \texttt{PhongChieu}, \texttt{LichChieu}, \texttt{Ghe}, \texttt{HoaDon}, \texttt{ChiTietHoaDon}, \texttt{DoAn}, \texttt{ChiTietDoAn}, \texttt{KhuyenMai} và \texttt{NhatKyHeThong}. Mỗi thực thể ánh xạ tương ứng với một bảng trong cơ sở dữ liệu \texttt{QuanLyRapPhim}. Toàn bộ ánh xạ này được thể hiện trong \texttt{QuanLyRapPhimContext.cs} thông qua phương thức \texttt{OnModelCreating()}.
\end{de}

\vspace{0.5cm}

\noindent Cơ sở dữ liệu gồm các bảng chính với các mối quan hệ khóa ngoại được mô tả trong Bảng~\ref{table_db}.
...
\begin{table}[H]
    \begin{center}
        \begin{tabular}{|l|l|l|}
            ...
            \hline
            NhanVien    & MaNV         & Tài khoản nhân viên/quản lý, chứa hash BCrypt \\
            \hline
            KhachHang   & MaKH         & Thông tin khách hàng \\
            \hline
            Phim        & MaPhim       & Thông tin bộ phim (Tên, Thể loại, Đạo diễn) \\
            \hline
            PhongChieu  & MaPhong      & Phòng chiếu phim \\
            \hline
            LichChieu   & MaLich       & Lịch chiếu: Phim, Phòng, Ngày, Giờ, Giá vé \\
            \hline
            Ghe         & MaGhe        & Ghế trong phòng, loại ghế \\
            \hline
            HoaDon      & MaHD         & Hóa đơn đặt vé \\
            \hline
            ChiTietHoaDon & MaCTHD     & Chi tiết hóa đơn (Vé xem phim) liên kết Lịch chiếu, Ghế \\
            \hline
            DoAn        & MaDoAn       & Dịch vụ bắp nước, đồ ăn \\
            \hline
            ChiTietDoAn & (MaHD, MaDoAn) & Chi tiết đồ ăn trong hóa đơn \\
            \hline
            KhuyenMai   & MaKM         & Chương trình khuyến mãi (tên, \% giảm, điều kiện, thời hạn) \\
            \hline
            NhatKyHeThong & MaNhatKy   & Nhật ký hoạt động hệ thống (Code First) \\
            \hline
        \end{tabular}
    \end{center}
...
\begin{lem}\label{lem1}
    ...
    \begin{itemize}
        \item \textbf{Khóa ngoại (FK):} \texttt{ChiTietHoaDon.MaGhe} $\to$ \texttt{Ghe.MaGhe}; \texttt{ChiTietHoaDon.MaHD} $\to$ \texttt{HoaDon.MaHD}; \texttt{ChiTietHoaDon.MaLich} $\to$ \texttt{LichChieu.MaLich}; \texttt{LichChieu.MaPhim} $\to$ \texttt{Phim.MaPhim}; v.v.
        \item \textbf{Duy nhất (Unique Index):} \texttt{NhanVien.TaiKhoan} là duy nhất; cặp \texttt{(ChiTietHoaDon.MaLich, ChiTietHoaDon.MaGhe)} không trùng lặp (\texttt{UC\_LichChieu\_Ghe}).
        \item \textbf{Giá trị mặc định (Default):} \texttt{HoaDon.NgayLap = GETDATE()}; \texttt{HoaDon.TongTien = 0}; \texttt{Ghe.DaDat = false}; \texttt{LichChieu.GiaVe = 0}.
        \item \textbf{Kiểu tiền tệ:} Các cột \texttt{GiaVe}, \texttt{TongTien}, \texttt{DonGiaBan} sử dụng kiểu \texttt{money} của SQL Server.
    \end{itemize}
\end{lem}
...
BEGIN
    ...
    FROM ChiTietHoaDon WHERE MaHD = @MaHD;
    
    SELECT @TongTienDV = ISNULL(SUM(DonGiaBan * SoLuong), 0)
    FROM ChiTietDoAn WHERE MaHD = @MaHD;
...
BEGIN
    ...
    SELECT p.MaPhim, p.TenPhim, p.ThoiLuong, 
           p.DaoDien, p.TheLoai, 
           p.NgayKhoiChieu, p.Poster
    FROM Phim p
    WHERE EXISTS (
        SELECT 1 FROM LichChieu lc 
        WHERE lc.MaPhim = p.MaPhim 
          AND CAST(lc.NgayChieu AS DATE) >= CAST(GETDATE() AS DATE)
    );
END
\end{lstlisting}
...
\noindent Trigger \texttt{trg\_NganXoaHoaDonCoCTHD} sử dụng \texttt{INSTEAD OF DELETE} để bảo vệ các hóa đơn đã được xuất vé:
...
BEGIN
    ...
    BEGIN
        RAISERROR(N'Không thể xóa hóa đơn đã có chi tiết đặt vé!',
                  16, 1);
...
\begin{thm}\label{thm_transaction}
    Toàn bộ tiến trình thanh toán (lưu \texttt{HoaDon}, \texttt{ChiTietHoaDon} và \texttt{ChiTietDoAn}) được bao bọc trong một \textbf{Database Transaction} của Entity Framework Core. Nếu bất kỳ bước nào thất bại, toàn bộ sẽ bị \texttt{Rollback}, đảm bảo tính nguyên tử (Atomicity).
\end{thm}
...
\begin{table}[H]
    \begin{center}
        \begin{tabular}{|l|l|}
            ...
            \hline
            \multicolumn{2}{|c|}{\textit{Khu vực Khách hàng (Controllers/)}} \\
            \hline
            \texttt{AccountController}           & Đăng nhập, đăng nhập bằng Google OAuth, đăng ký, đăng xuất \\
            \hline
            ...
            \hline
            \multicolumn{2}{|c|}{\textit{Khu vực Quản trị (Areas/Admin/Controllers/)}} \\
            \hline
            \texttt{Admin/PhimController}        & Quản trị phim: CRUD + upload ảnh lên \texttt{/images/phim/} \\
            \hline
            \texttt{Admin/PhongChieuController}  & Quản trị phòng chiếu: CRUD, tự động sinh sơ đồ ghế \\
            \hline
            \texttt{Admin/SuatChieuController}   & Quản trị lịch chiếu: CRUD, tự tính giờ kết thúc \\
            \hline
            \texttt{Admin/DichVuController}      & Quản trị dịch vụ đồ ăn/nước uống: CRUD \\
            \hline
            \texttt{Admin/DoanhThuController}    & Báo cáo doanh thu qua biểu đồ Chart.js và xuất file Excel \\
            \hline
            \texttt{Admin/HomeController}        & Dashboard: tổng phim, vé bán, doanh thu, khách hàng \\
            \hline
            \texttt{Admin/KhachHangController}   & Quản lý khách hàng: xem, sửa, xóa \\
            \hline
            \texttt{Admin/NhanVienController}    & Quản lý nhân viên: CRUD \\
            \hline
            \multicolumn{2}{|c|}{\textit{Khu vực Nhân viên (Areas/NhanVien/Controllers/)}} \\
            \hline
            \texttt{NhanVien/HomeController}     & Dashboard nhân viên \\
            \hline
            \texttt{NhanVien/PhimController}     & Quản lý phim phía nhân viên \\
            \hline
            \texttt{NhanVien/SuatChieuController} & Quản lý lịch chiếu phía nhân viên \\
            \hline
            \texttt{NhanVien/DichVuController}   & Quản lý dịch vụ phía nhân viên \\
            \hline
            \texttt{NhanVien/DoanhThuController} & Xem báo cáo doanh thu phía nhân viên \\
            \hline
        \end{tabular}
    \end{center}
...
\begin{table}[H]
    \begin{center}
        \begin{tabular}{|l|l|}
            ...
            \hline
            \texttt{IPhimBUS / PhimBUS}               & CRUD phim, tìm kiếm, gọi SP, lấy ghế theo lịch chiếu \\
            \hline
            \texttt{IHoaDonBUS / HoaDonBUS}           & Lưu hóa đơn + vé + dịch vụ trong Transaction \\
            \hline
            \texttt{IKhachHangBUS / KhachHangBUS}     & Đăng ký, đăng nhập (\texttt{BCrypt} + Google), profile, đổi MK \\
            \hline
            \texttt{INhanVienBUS / NhanVienBUS}       & Đăng nhập nhân viên, CRUD nhân viên, đổi mật khẩu \\
            \hline
            \texttt{IDoAnBUS / DoAnBUS}               & Lấy danh sách dịch vụ, lấy theo ID \\
            \hline
            \texttt{IKhuyenMaiBUS / KhuyenMaiBUS}     & CRUD chương trình khuyến mãi \\
            \hline
            \texttt{IPhongChieuBUS / PhongChieuBUS}   & CRUD phòng chiếu, tự động sinh ghế theo sức chứa \\
            \hline
        \end{tabular}
    \end{center}
...
\begin{table}[H]
    \begin{center}
        \begin{tabular}{|l|l|}
            ...
            \hline
            \texttt{PhimDTO}              & Thông tin phim + danh sách lịch chiếu hiển thị \\
            \hline
            \texttt{LichChieuDTO}         & Mã lịch, giờ chiếu, giá vé \\
            \hline
            \texttt{LichChieuHienThiDTO}  & Mã lịch, chuỗi giờ hiển thị (dạng \texttt{hh:mm}) \\
            \hline
            \texttt{GheDTO}               & Mã ghế, hàng, số ghế, loại ghế, trạng thái đã đặt \\
            \hline
            \texttt{DoAnDTO}              & Thông tin dịch vụ, số lượng chọn, thành tiền \\
            \hline
            \texttt{CartItemDTO}          & Giỏ hàng: phim, ghế, dịch vụ, tổng tiền \\
            \hline
            \texttt{HoaDonDTO}            & Hóa đơn đầy đủ cho E-Ticket (phim, ghế, dịch vụ) \\
            \hline
            \texttt{ChiTietDoAnDTO}       & Chi tiết dịch vụ trong hóa đơn (tên, số lượng, giá) \\
            \hline
            \texttt{KhachHangDTO}         & Thông tin khách hàng + lịch sử hóa đơn \\
            \hline
            \texttt{NhanVienDTO}          & Thông tin nhân viên (họ tên, chức vụ, phân quyền) \\
            \hline
            \texttt{KhuyenMaiDTO}         & Khuyến mãi (tên, \% giảm, điều kiện, thời hạn) \\
            \hline
            \texttt{PhongChieuDTO}        & Phòng chiếu (tên, loại, sức chứa, số ghế đã tạo) \\
            \hline
            \texttt{DoiMatKhauRequest}    & Mật khẩu hiện tại, mật khẩu mới, xác nhận mật khẩu mới \\
            \hline
        \end{tabular}
    \end{center}
...
\begin{itemize}
    \item \textbf{Entity Framework Core (Database First):} Lớp \texttt{QuanLyRapPhimContext} kế thừa \texttt{DbContext} định nghĩa 12 \texttt{DbSet<T>} (bao gồm cả \texttt{KhuyenMai} và \texttt{NhatKyHeThong}) và ánh xạ cột qua \texttt{OnModelCreating()}. Chuỗi kết nối đọc từ \texttt{appsettings.json} qua DI.
...
\begin{table}[H]
    \begin{center}
        \begin{tabular}{|c|l|l|}
            ...
            \hline
            \texttt{GET}    & \texttt{/api/phimapi/adonet-dataset} & DataSet (Phim + LichChieu) \\
            \hline
            ...
            \hline
            \hline
            \texttt{GET}    & \texttt{/api/doanapi}              & Lấy tất cả đồ ăn (bắp nước) \\
            \hline
            \texttt{GET}    & \texttt{/api/doanapi/\{id\}}       & Chi tiết 1 đồ ăn \\
            \hline
            \texttt{POST}   & \texttt{/api/doanapi}              & Thêm đồ ăn \\
            \hline
            \texttt{PUT}    & \texttt{/api/doanapi/\{id\}}       & Cập nhật đồ ăn \\
            \hline
            \texttt{DELETE} & \texttt{/api/doanapi/\{id\}}       & Xóa đồ ăn (kiểm tra rỗng) \\
            \hline
            \hline
            \texttt{GET}    & \texttt{/api/lichchieuapi}         & Lấy danh sách lịch chiếu \\
            \hline
            \texttt{GET}    & \texttt{/api/lichchieuapi/\{id\}}  & Chi tiết 1 lịch chiếu \\
            \hline
            \texttt{GET}    & \texttt{/api/lichchieuapi/phim/\{maPhim\}} & Lịch chiếu theo phim (từ hôm nay) \\
            \hline
            \texttt{POST}   & \texttt{/api/lichchieuapi}         & Thêm lịch chiếu \\
            \hline
            \texttt{PUT}    & \texttt{/api/lichchieuapi/\{id\}}  & Cập nhật lịch chiếu \\
            \hline
            \texttt{DELETE} & \texttt{/api/lichchieuapi/\{id\}}  & Xóa lịch chiếu (kiểm tra tồn tại vé) \\
            \hline
        \end{tabular}
    \end{center}
...
\begin{itemize}
    \item \textbf{Lập trình CSDL T-SQL:} Xây dựng 12 bảng quan hệ trong SQL Server với đầy đủ View (\texttt{vw\_DoanhThuTheoPhim}), Function (\texttt{fn\_TinhTongTienHoaDon}), Stored Procedure (\texttt{sp\_LayDanhSachPhimDangChieu}), Trigger (\texttt{trg\_NganXoaHoaDonCoCTHD}) và Transaction (EF Core \texttt{BeginTransaction/Commit/Rollback}).
    ...
    \item \textbf{Công nghệ \& Tối ưu:} Tích hợp \textbf{IMemoryCache} để tăng tốc load danh sách phim, xuất báo cáo ra file Excel với thư viện \textbf{ClosedXML}, kiểm thử mã nguồn với \textbf{xUnit và Moq} (hoàn thành 20 Unit Tests bao phủ tầng BUS: PhimBUS, DoAnBUS, NhanVienBUS, PhongChieuBUS).
    \item \textbf{ADO.NET \& LINQ:} Triển khai cả mô hình kết nối và phi kết nối (\texttt{DataSet}, \texttt{SqlDataReader}). Sử dụng LINQ to XML cho tính năng Export/Import.
    \item \textbf{Web API RESTful:} 3 Controller API trả dữ liệu JSON chuẩn REST.
    \item \textbf{Tính năng người dùng:} \textbf{Đăng nhập một chạm bằng Google OAuth 2.0}, đặt vé với hệ thống phòng chiếu \textbf{sinh sơ đồ ghế động}, đồng bộ hóa khóa ghế realtime bằng \textbf{SignalR}, \textbf{thanh toán trực tuyến bằng cổng VNPay Sandbox}, E-Ticket có mã QR.
    \item \textbf{Giao diện (UI/UX):} Ứng dụng công nghệ \textbf{GSAP} mang lại trải nghiệm hoạt ảnh mượt mà, kết hợp hệ thống điều hướng hồ sơ người dùng (User Profile Navigation) trực quan.
    \item \textbf{Bảo mật hệ thống:} Mã hóa mật khẩu người dùng bằng thư viện \textbf{BCrypt}.
\end{itemize}
...
\begin{itemize}
    ...
    \item Container hóa hệ thống bằng \textbf{Docker} và triển khai CI/CD trên GitHub Actions.
\end{itemize}
